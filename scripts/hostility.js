...
export function hostileTo(proxy, character) {
  ...
  const sources = [proxy.uuid, `Actor.${proxy.id}`, proxy.getFlag("core", "sourceId"), proxy._stats?.compendiumSource], state = Runtime.read();
...
export function registerHostility() {
  Hooks.on("drawToken", token => {
    ...
    if (token[`_${MODULE}`]) return;
    token[`_${MODULE}`] = true;
    token._getBorderColor = function (...args) {
      const controlled = canvas.tokens.controlled.filter(row => row.actor && !row.actor.isToken), character = controlled.length === 1 ? controlled[0].actor : game.user.character;
